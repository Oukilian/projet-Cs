...
let insert = fun ab x ->
	if (est_dans ab x) then
		...
		let rec aux = fun abb xx ->
			match abb with
			| Vide -> Noeud(Vide,(Element xx),Rouge,Vide)
...
let rec all_int = function ens ->
	match ens with
	| Vide -> []
	| Noeud(ag,x,_,ad) -> (all_int ag)@[x]@(all_int ad)
	|_ -> failwith "impossible value";;

let union = fun ens1 ens2 ->
	let rec aux = fun ens1 ens2 acc ->
		match ens1, ens2 with
		| [], [] -> List.rev(acc)
		| [], l -> List.rev(List.rev(l)@acc)
		| l, [] -> List.rev(List.rev(l)@acc)
		| a::l1, b::l2 when a > b -> aux (a::l1) (l2) (b::acc)
		| a::l1,b::l2 when a = b -> aux l1 l2 (a::acc)
		| a::l1,b::l2 -> aux (a::l1) l2 (b::acc)
	in aux (all_int ens1) (all_int ens2) [];;
...
let ensInt =  function () ->
	let rec aux = fun ab x ->
		match ab with
		| Vide -> aux (insert ab x) max_int
		| a -> insert a x
	in aux Vide 0;;
 
12

let ab = Noeud(Vide,(Element 0),Rouge,Noeud(Vide,element(12),Rouge,Vide));;
insert ab 15;;
let n = ensInt ();;

let intersection = fun ens1 ens2 ->
	let rec aux = fun ens1 ens2 acc ->
		match ens1, ens2 with
		| [], [] -> List.rev(acc)
		| [], l -> List.rev(List.rev(l)@acc)
		| l, [] -> List.rev(List.rev(l)@acc)
		| a::l1, b::l2 when a < b -> aux l1 (b::l2) acc
		| a::l1,b::l2 when a = b -> aux l1 l2 (a::acc)
		| a::l1,b::l2 -> aux (a::l1) l2 (acc)
	in aux (all_int ens1) (all_int ens2) [];;
